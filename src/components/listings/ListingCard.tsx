      ...
      <div className="card overflow-hidden hover:shadow-lg transition-shadow">
        ...
        <div className="p-2 sm:p-4">
          ...
          <div className="flex gap-1.5 sm:gap-2 mb-2 sm:mb-3">
            {listing.osVersion && (
              <div className="flex-1 bg-gradient-to-r from-gray-900 to-gray-700 dark:from-gray-700 dark:to-gray-600 text-white rounded-md sm:rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 text-center">
                <p className="text-[8px] sm:text-[10px] tracking-wide text-gray-300">iOS</p>
                <p className="text-sm sm:text-lg font-bold leading-tight">{listing.osVersion}</p>
              </div>
            )}
            <div className={`flex-1 rounded-md sm:rounded-lg px-2 sm:px-3 py-1.5 sm:py-2 text-center ${
              jailbreak && jailbreak.label === 'Jailbreakable'
                ? 'bg-gradient-to-r from-purple-600 to-purple-500 text-white'
                : 'bg-gray-100 dark:bg-gray-700 text-gray-600 dark:text-gray-300'
            }`}>
              <p className="text-[8px] sm:text-[10px] tracking-wide opacity-75">Jailbreakable?</p>
          ...
          {/* Title */}
          <h3 className="text-xs sm:text-base font-medium text-gray-900 dark:text-white mb-1.5 sm:mb-2 line-clamp-2 group-hover:text-primary-600 transition-colors">
            {listing.title}
          </h3>

          {/* Condition & checkm8 badges */}
          <div className="flex flex-wrap gap-1 sm:gap-1.5 mb-2 sm:mb-3">
            <span className={`badge text-[10px] sm:text-xs ${condition.color}`}>{condition.label}</span>
            {listing.bootromExploit && (
              <span className="badge text-[10px] sm:text-xs bg-amber-100 text-amber-800">checkm8</span>
            )}
          </div>
